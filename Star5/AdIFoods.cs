...
namespace Star5
{
    public partial class AdIFoods : Form
    {
        ...
        void BindGridView()
        {
            ...
            string query = "select * from I_Food";
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "insert into I_Food values (@id,@name,@price,@image)";
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "update I_Food set id=@id,name=@name,price=@price,image=@image where id=@id";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@id", textBox1.Text);
            cmd.Parameters.AddWithValue("@name", textBox2.Text);
            cmd.Parameters.AddWithValue("@price", textBox3.Text);
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "delete from I_Food where id=@id";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@id", textBox1.Text);
            ...
            if (a > 0)
            {
                MessageBox.Show("Food Deleted Successfully", "Success", MessageBoxButtons.OK);
                BindGridView();
                ResetControl();
            }
            else
            {
                MessageBox.Show("Food Did Not Deleted", "Failed", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
